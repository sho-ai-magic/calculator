...
function initialize() {
    ...
    if(elements.gachaCloseResult) elements.gachaCloseResult.addEventListener('click', () => { SoundManager.click(); if (elements.gachaResultOverlay) elements.gachaResultOverlay.classList.add('hidden'); if (elements.gachaAnimArea) elements.gachaAnimArea.textContent = "🔮"; updateAllUI(); });
    ...
    if(elements.confirmGachaYes) elements.confirmGachaYes.addEventListener('click', () => { SoundManager.click(); if (elements.gachaConfirmModal) elements.gachaConfirmModal.classList.remove('hidden'); updateAllUI(); showScreen('record'); });

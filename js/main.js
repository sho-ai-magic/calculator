...
function initialize() {
    ...
    if(elements.reviewQuizBtn) elements.reviewQuizBtn.addEventListener('click', () => { SoundManager.init(); SoundManager.click(); startReviewQuiz(); });
    if(elements.closeReviewEmptyBtn) elements.closeReviewEmptyBtn.addEventListener('click', () => { SoundManager.click(); if (elements.reviewEmptyModal) elements.reviewEmptyModal.classList.add('hidden'); });
    ...
    if(elements.closeLevelUpBtn) elements.closeLevelUpBtn.addEventListener('click', () => {
        SoundManager.click();
        if (elements.levelUpModal) elements.levelUpModal.classList.add('hidden');
        // 復習でまちがいノートが空になった場合はお祝いを優先（ガチャ確認は出さない）
        if (quizState.mode === 'review' && userData.wrongProblems.length === 0) {
            if (elements.reviewEmptyModal) elements.reviewEmptyModal.classList.remove('hidden');
        } else if (quizState.mode !== 'review' && quizState.correctCount === quizState.problems.length && elements.gachaConfirmModal) {
            elements.gachaConfirmModal.classList.remove('hidden');
        }
    });
